...
{
    return hash_crc32c_one(trans_seed, (uint64_t)laddr.ip | ((uint64_t)laddr.port << 32) |
                                           ((uint64_t)proto << 48));
}

static inline uint32_t trans_hash_5tuple(uint8_t proto, struct netaddr laddr, struct netaddr raddr)
{
    return hash_crc32c_two(trans_seed, (uint64_t)laddr.ip | ((uint64_t)laddr.port << 32),
...
int trans_table_add(struct trans_entry *e)
{
    ...
    assert(e->match == TRANS_MATCH_3TUPLE || e->match == TRANS_MATCH_5TUPLE);
    ...
    rcu_hlist_for_each(&trans_tbl[idx], node, true)
    {
        ...
        if (e->match == TRANS_MATCH_3TUPLE && e->proto == pos->proto &&
            e->laddr.ip == pos->laddr.ip && e->laddr.port == pos->laddr.port) {
            spin_unlock_np(&trans_lock);
            return -EADDRINUSE;
        } else if (e->proto == pos->proto && e->laddr.ip == pos->laddr.ip &&
                   e->laddr.port == pos->laddr.port && e->raddr.ip == pos->raddr.ip &&
    ...
    atomic_store_rel(&ephemeral_offset, ephemeral_offset + 1);
    spin_unlock_np(&trans_lock);
...
int trans_table_add_with_ephemeral_port(struct trans_entry *e)
{
    ...
    e->laddr.port = 0;
    offset = trans_hash_5tuple(e->proto, e->laddr, e->raddr) + atomic_load_acq(&ephemeral_offset);
    while (next_ephemeral < num_ephemeral) {
        uint32_t port = MIN_EPHEMERAL + (next_ephemeral++ + offset) % num_ephemeral;
        e->laddr.port = port;
...
static struct trans_entry *trans_lookup(struct mbuf *m)
{
    ...
    if (unlikely(iphdr->proto != IPPROTO_UDP && iphdr->proto != IPPROTO_TCP))
        return NULL;
    ...
    rcu_hlist_for_each(&trans_tbl[hash % TRANS_TBL_SIZE], node, false)
    {
        ...
        if (e->match != TRANS_MATCH_5TUPLE)
            continue;
        if (e->proto == iphdr->proto && e->laddr.ip == laddr.ip && e->laddr.port == laddr.port &&
...
int trans_init(void)
{
    ...
    for (i = 0; i < TRANS_TBL_SIZE; i++) rcu_hlist_init_head(&trans_tbl[i]);

    trans_seed = rand_crc32c(0x48FA8BC1 ^ (uint32_t)proc->pid);
    return 0;
}
