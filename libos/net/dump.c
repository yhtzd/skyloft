...
void dump_eth_pkt(int loglvl, struct eth_hdr *hdr)
{
    ...
    logk(loglvl, "\tdst MAC: %02X:%02X:%02X:%02X:%02X:%02X", dmac->addr[0], dmac->addr[1],
         dmac->addr[2], dmac->addr[3], dmac->addr[4], dmac->addr[5]);
    logk(loglvl, "\tsrc MAC: %02X:%02X:%02X:%02X:%02X:%02X", smac->addr[0], smac->addr[1],
         smac->addr[2], smac->addr[3], smac->addr[4], smac->addr[5]);
    logk(loglvl, "\tframe type: %x", ntoh16(hdr->type));
}
...
void dump_arp_pkt(int loglvl, struct arp_hdr *arphdr, struct arp_hdr_ethip *ethip)
{
    ...
    logk(loglvl, "ARP packet dump: op %s", (op == ARP_OP_REQUEST) ? "request" : "response");

    if (!ethip) {
        logk(loglvl, "\tunsupported htype %d, ptype %d", ntoh16(arphdr->htype),
             ntoh16(arphdr->ptype));
        return;
    }

    logk(loglvl, "\tsender MAC:\t%02X:%02X:%02X:%02X:%02X:%02X", smac->addr[0], smac->addr[1],
         smac->addr[2], smac->addr[3], smac->addr[4], smac->addr[5]);
    logk(loglvl, "\tsender IP:\t%d.%d.%d.%d", ((sip >> 24) & 0xff), ((sip >> 16) & 0xff),
         ((sip >> 8) & 0xff), (sip & 0xff));
    logk(loglvl, "\ttarget MAC:\t%02X:%02X:%02X:%02X:%02X:%02X", tmac->addr[0], tmac->addr[1],
         tmac->addr[2], tmac->addr[3], tmac->addr[4], tmac->addr[5]);
    logk(loglvl, "\ttarget IP:\t%d.%d.%d.%d", ((tip >> 24) & 0xff), ((tip >> 16) & 0xff),
         ((tip >> 8) & 0xff), (tip & 0xff));
}
...
void dump_udp_pkt(int loglvl, uint32_t saddr, struct udp_hdr *udp_hdr, void *data)
{
    ...
    logk(loglvl, "UDP packet received from %s:%d on port %d", sip, sport, dport);
    for (c = 0; c < len;) {
...
{
    snprintf(str, IP_ADDR_STR_LEN, "%d.%d.%d.%d", ((addr >> 24) & 0xff), ((addr >> 16) & 0xff),
