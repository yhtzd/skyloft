...
void net_send_ping(uint16_t seq_num, uint32_t daddr)
{
    ...
    log_debug("ping: sending ping with id %u, seq_num %u to %u", ping_id, seq_num, daddr);
...
void net_recv_ping(const struct ping_payload *payload, const struct icmp_pkt *icmp_pkt)
{
    ...
    log_debug("ping: received ping with seq_num %u, latency %u us", icmp_pkt->icmp_seq, latency_us);
}
