...
static void net_rx_icmp_echo(struct mbuf *m_in, const struct icmp_pkt *in_icmp_pkt,
                             const struct ip_hdr *in_iphdr, uint16_t len)
{
    ...
    out_icmp_hdr->cksum = 0;
    out_icmp_hdr->cksum = cksum_internet((char *)out_icmp_hdr, len);
...
static void net_rx_icmp_echo_reply(struct mbuf *m, const struct icmp_pkt *icmp_pkt, uint16_t len)
{
                ...
                uint16_t seq)
{
    ...
    icmp_pkt->hdr.cksum = 0;
    icmp_pkt->hdr.cksum = cksum_internet((char *)icmp_pkt, mbuf_length(m));
