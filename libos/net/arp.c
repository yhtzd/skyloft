...
#define ARP_RETRY_TIME     USEC_PER_SEC
#define ARP_REPROBE_TIME   (10 * USEC_PER_SEC)
...
int arp_static_count;
struct arp_static_entry static_entries[MAX_ARP_STATIC_ENTRIES];
...
static void insert_entry(struct arp_entry *e, int idx)
{
    ...
    if (unlikely(!worker_running && e->state != ARP_STATE_STATIC)) {
        worker_running = true;
        BUG_ON(sl_task_spawn(arp_worker, NULL, 0));
    }
}
...
void arp_send(uint16_t op, struct eth_addr dhost, uint32_t daddr)
{
    ...
    arp_hdr_ethip->sender_mac = io->mac;
    arp_hdr_ethip->sender_ip = hton32(io->addr);
...
static void arp_age_entry(uint64_t us, struct arp_entry *e)
{
    /* check if this entry has timed out */
    if (us - e->ts < ((e->state == ARP_STATE_VALID) ? ARP_REPROBE_TIME : ARP_RETRY_TIME))
        return;
...
static void arp_update(uint32_t daddr, struct eth_addr dhost)
{
    ...
    if (!e) {
        ...
        insert_entry(e, idx);
    } else if (atomic_load_acq(&e->state) == ARP_STATE_STATIC) {
        spin_unlock_np(&arp_lock);
        return;
    }
    e->eth = dhost;
    e->ts = now_us();
    atomic_store_rel(&e->state, ARP_STATE_VALID);
...
void net_rx_arp(struct mbuf *m)
{
    ...
    if (ntoh16(arp_hdr->htype) != ARP_HTYPE_ETHER || ntoh16(arp_hdr->ptype) != ETHTYPE_IP ||
        arp_hdr->hlen != sizeof(struct eth_addr) || arp_hdr->plen != sizeof(uint32_t))
        goto out;
    ...
    if (am_target && op == ARP_OP_REQUEST) {
        log_debug("arp: responding to arp request "
                  ...
                  ((sender_ip >> 24) & 0xff), ((sender_ip >> 16) & 0xff), ((sender_ip >> 8) & 0xff),
                  (sender_ip & 0xff), sender_mac.addr[0], sender_mac.addr[1], sender_mac.addr[2],
                  sender_mac.addr[3], sender_mac.addr[4], sender_mac.addr[5]);
...
int arp_lookup(uint32_t daddr, struct eth_addr *dhost_out, struct mbuf *m)
{
    ...
    rcu_read_lock();
    e = lookup_entry(idx, daddr);
    if (likely(e && atomic_load_acq(&e->state) != ARP_STATE_PROBING)) {
        *dhost_out = e->eth;
        rcu_read_unlock();
        return 0;
    }
...
int arp_init(void)
{
    ...
    for (i = 0; i < ARP_TABLE_CAPACITY; i++) rcu_hlist_init_head(&arp_tbl[i]);
