...
static int udp_send_raw(struct mbuf *m, size_t len, struct netaddr laddr, struct netaddr raddr)
{
...
{
    udp_conn_t *c = container_of(e, udp_conn_t, e);
    struct task *t;
    ...
    /* wake up a waiter */
    t = waitq_signal(&c->inq_wq, &c->inq_lock);
    spin_unlock_np(&c->inq_lock);
...
static void udp_init_conn(udp_conn_t *c)
{
    c->shutdown = false;
...
{
    udp_conn_t *c = container_of(h, udp_conn_t, e.rcu);
    sfree(c);
}

static void udp_release_conn(udp_conn_t *c)
{
...
ssize_t udp_read_from(udp_conn_t *c, void *buf, size_t len, struct netaddr *raddr)
{
    ...
    if (raddr) {
        ...
        if (c->e.match == TRANS_MATCH_5TUPLE) {
            assert(c->e.raddr.ip == raddr->ip && c->e.raddr.port == raddr->port);
        }
    }
    mbuf_free(m);
    return ret;
...
{
    udp_conn_t *c = (udp_conn_t *)m->release_data;
    struct task *t = NULL;
    bool free_conn;
    ...
    if (!c->shutdown)
        t = waitq_signal(&c->outq_wq, &c->outq_lock);
...
ssize_t udp_write_to(udp_conn_t *c, const void *buf, size_t len, const struct netaddr *raddr)
{
    ...
    /* block until there is an actionable event */
    while (c->outq_len >= c->outq_cap && !c->shutdown) waitq_wait(&c->outq_wq, &c->outq_lock);
...
void __udp_shutdown(udp_conn_t *c)
{
    spin_lock_np(&c->inq_lock);
    spin_lock_np(&c->outq_lock);
    BUG_ON(c->shutdown);
...
{
    udp_spawner_t *s = container_of(e, udp_spawner_t, e);
    ...
    t = task_create_with_buf((thread_fn_t)s->fn, (void **)&d, sizeof(*d));
    if (unlikely(!t)) {
        mbuf_free(m);
        return;
    }
    ...
    if (unlikely(task_enqueue(current_cpu_id(), t))) {
        log_err("udp: failed to spawn udp packet receiver");
        mbuf_free(m);
        task_free(t);
        return;
    }
...
{
    udp_spawner_t *s = container_of(h, udp_spawner_t, e.rcu);
    sfree(s);
}
