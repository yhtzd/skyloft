...
{
    if (atomic_dec_zero(&m->ref))
        net_tx_release_mbuf(m);
}

static struct tcp_hdr *tcp_push_tcphdr(struct mbuf *m, tcp_conn_t *c, uint8_t flags, uint16_t l4len)
{
    struct tcp_hdr *tcphdr;
    uint64_t rcv_nxt_wnd = atomic_load_acq(&c->pcb.rcv_nxt_wnd);
    ...
    tcphdr->sum =
        ipv4_phdr_cksum(IPPROTO_TCP, c->e.laddr.ip, c->e.raddr.ip, sizeof(struct tcp_hdr) + l4len);
    return tcphdr;
}
...
int tcp_tx_raw_rst(struct netaddr laddr, struct netaddr raddr, tcp_seq seq)
{
    ...
    tcphdr->win = hton16(0);
    tcphdr->sum = ipv4_phdr_cksum(IPPROTO_TCP, laddr.ip, raddr.ip, sizeof(struct tcp_hdr));
...
int tcp_tx_ack(tcp_conn_t *c)
{
    ...
    m->txflags = OLFLAG_TCP_CHKSUM;
    m->seg_seq = atomic_load_acq(&c->pcb.snd_nxt);
...
int tcp_tx_ctl(tcp_conn_t *c, uint8_t flags)
{
    ...
    atomic_store_rel(&c->pcb.snd_nxt, c->pcb.snd_nxt + 1);
    list_add_tail(&c->txq, &m->link);
    m->timestamp = now_us();
    atomic_store(&m->ref, 2);
    m->release = tcp_tx_release_mbuf;
    tcp_debug_egress_pkt(c, m);
    ret = net_tx_ip(m, IPPROTO_TCP, c->e.raddr.ip);
    if (unlikely(ret)) {
        /* pretend the packet was sent */
        atomic_store(&m->ref, 1);
    }
    return ret;
}
...
ssize_t tcp_tx_send(tcp_conn_t *c, const void *buf, size_t len, bool push)
{
    ...
    while (pos < end) {
        ...
        atomic_store_rel(&c->pcb.snd_nxt, c->pcb.snd_nxt + seglen);
        pos += seglen;

        /* if not pushing, keep the last buffer for later */
        if (!push && pos == end && mbuf_length(m) - sizeof(struct tcp_hdr) < TCP_MSS) {
            c->tx_pending = m;
            break;
        }
...
static int tcp_tx_retransmit_one(tcp_conn_t *c, struct mbuf *m)
{
     ...
     */
    if (unlikely(atomic_load(&m->ref) != 1)) {
        struct mbuf *newm = net_tx_alloc_mbuf();
        if (unlikely(!newm))
            return -ENOMEM;
        memcpy(mbuf_put(newm, l4len), mbuf_transport_offset(m) + sizeof(struct tcp_hdr), l4len);
        newm->flags = m->flags;
    ...
    uint32_t una = atomic_load_acq(&c->pcb.snd_una);
...
void tcp_tx_retransmit(tcp_conn_t *c)
{
    ...
    list_for_each(&c->txq, m, link)
    {
        ...
        if (wraps_gte(atomic_load_acq(&c->pcb.snd_una), m->seg_end))
            continue;
