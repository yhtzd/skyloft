...
static inline void waitq_wait(waitq_t *q, spinlock_t *l)
{
    ...
    list_add_tail(&q->waiters, &task->link);
    task_block(l);
    spin_lock_np(l);
}
 ...
 * @l: a held spinlock protecting the wake queue and the condition
 */
static inline struct task *waitq_signal(waitq_t *q, spinlock_t *l)
{
    assert_spin_lock_held(l);
    return list_pop(&q->waiters, struct task, link);
}
 ...
 * section.
 */
static inline void waitq_signal_finish(struct task *t)
{
    if (t)
        task_wakeup(t);
}
...
{
    struct task *t = waitq_signal(q, l);
...
{
    while (true) {
        struct task *t = list_pop(&q->waiters, struct task, link);
        if (!t)
            break;
        task_wakeup(t);
    }
...
{
    while (true) {
        struct task *t = list_pop(waiters, struct task, link);
        if (!t)
            break;
        task_wakeup(t);
    }
