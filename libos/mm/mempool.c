...
static void mempool_common_check(struct mempool *m, void *item)
{
    ...
    assert((start & (m->pg_sz - 1)) % m->item_len == 0);
}
...
static int mempool_populate(struct mempool *m, void *buf, size_t len, size_t pg_sz, size_t item_len)
{
    size_t items_per_page = pg_sz / item_len;
    size_t nr_pages = len / pg_sz;
    size_t i, j;
    ...
    for (i = 0; i < nr_pages; i++) {
        for (j = 0; j < items_per_page; j++) {
            m->free_items[m->capacity++] = (char *)buf + pg_sz * i + item_len * j;
        }
    }
 ...
 * @pg_sz: the size of the pages in the buffer region (must be uniform)
 * @item_len: the length of each item in the pool
 */
int mempool_create(struct mempool *m, void *buf, size_t len, size_t pg_sz, size_t item_len)
{
    if (item_len == 0 || !is_power_of_two(pg_sz) || len % pg_sz != 0)
        return -EINVAL;
    ...
    return mempool_populate(m, buf, len, pg_sz, item_len);
}
...
static const struct tcache_ops mempool_tcache_ops = {
    .alloc = mempool_tcache_alloc,
    .free = mempool_tcache_free,
};

struct tcache *mempool_create_tcache(struct mempool *m, const char *name, unsigned int mag_size)
{
