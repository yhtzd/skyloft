...
#define SLAB_4KB_LIMIT (align_down(PGSIZE_4KB / (SLAB_PARTIAL_THRESH * 2), __WORD_SIZE))
#define SLAB_2MB_LIMIT (align_down(PGSIZE_2MB / (SLAB_PARTIAL_THRESH * 2), __WORD_SIZE))

static DEFINE_LIST_HEAD(slab_list);
...
static struct slab_node early_slab_nodes[MAX_NUMA];
static struct slab_node early_smpage_nodes[MAX_NUMA];

static void __slab_create_node(struct slab_node *n, int numa_node, size_t size, int offset,
                               int flags, int nr_elems)
{
...
static void __slab_destroy_node(struct slab_node *n)
{
    ...
    list_for_each_safe(&n->full_list, pg, pg_next, link) page_put(pg);
    list_for_each_safe(&n->partial_list, pg, pg_next, link) page_put(pg);
    if (n->cur_pg)
        page_put(n->cur_pg);
}

static int __slab_create(struct slab *s, const char *name, size_t size, int offset, int flags,
                         int nr_elems)
{
    struct slab_node *n;
    int i;
...
fail:
    for (i--; i >= 0; i--) slab_free(&node_slab, s->nodes[i]);
    return -ENOMEM;
}

static void __slab_early_create(struct slab *s, struct slab_node *nodes, const char *name,
...
static void slab_item_check(struct slab_node *n, void *item)
{
    ...
    /* NUMA node checks */
    assert(n->numa_node == current_numa_node());
    assert(addr_to_numa_node(item) == current_numa_node());
...
static struct page *__slab_node_get_page(struct slab_node *n)
{
    ...
    if (!pg) {
        int pgsize = (n->flags & SLAB_FLAG_LGPAGE) ? PGSIZE_2MB : PGSIZE_4KB;
...
static void *__slab_node_alloc(struct slab_node *n)
{
    ...
    if (n->cur_pg->next) {
        ...
        assert(n->pg_off < ((n->flags & SLAB_FLAG_LGPAGE) ? PGSIZE_2MB : PGSIZE_4KB));
        hdr = (struct slab_hdr *)((char *)page_to_addr(n->cur_pg) + n->pg_off);
        n->pg_off += n->size;
    }
...
static int slab_tcache_alloc(struct tcache *tc, int nr, void **items)
{
    ...
    struct slab_node *n = s->nodes[current_numa_node()];
    int i;
...
fail:
    for (i--; i >= 0; i--) slab_node_free(n, items[i]);
    return -ENOMEM;
}
...
static void slab_tcache_free(struct tcache *tc, int nr, void **items)
{
    ...
    for (i = 0; i < nr; i++) slab_node_free(n, items[i]);
}
...
int slab_init(void)
{
    ...
    __slab_early_create(&node_slab, early_slab_nodes, "slab_node",
                        align_up(sizeof(struct slab_node), TCACHE_MIN_ITEM_SIZE), 0, 0,
                        PGSIZE_4KB / sizeof(struct slab_node));

    __slab_early_create(&smpage_slab, early_smpage_nodes, "smpage", PGSIZE_4KB, SMPAGE_META_LEN,
