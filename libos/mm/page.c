...
static void page_check(struct page *pg, size_t pgsize)
{
    ...
    assert(pg - page_tbl >= 0 && pg - page_tbl < (long)(LGPAGE_META_ENTS * MAX_NUMA));
}
...
static struct page *smpage_alloc_on_node(int numa_node)
{
    ...
    if (thread_init_done && current_numa_node() == numa_node) {
...
static void smpage_free(struct page *pg)
{
    ...
    int numa_node = addr_to_numa_node(addr);
...
{
    return page_alloc_on_node(pgsize, current_numa_node());
}
...
{
    return page_alloc_addr_on_node(pgsize, current_numa_node());
}
...
int page_init(void)
{
    ...
    addr = mmap(NULL, LGPAGE_META_LEN * MAX_NUMA + PGSIZE_2MB - 1, PROT_NONE,
    ...
    for (i = 0; i < MAX_NUMA; i++) {
        node = &lgpage_nodes[i];
        node->tbl = mem_map_anom((char *)addr + i * LGPAGE_META_LEN,
 ...
 */
int page_init_percpu(void)
{
    tcache_init_percpu(smpage_tcache, &smpage_pt);
    return 0;
}
