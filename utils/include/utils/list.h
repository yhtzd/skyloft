...
/**
 * DEFINE_LIST_HEAD - define and initialize an empty list_head
 * @name: the name of the list.
 *
 * The DEFINE_LIST_HEAD macro defines a list_head and initializes it to an empty
 ...
 *
 * Example:
 *	static DEFINE_LIST_HEAD(my_global_list);
 */
#define DEFINE_LIST_HEAD(name) struct list_head name = LIST_HEAD_INIT(name)
...
static inline void __list_add(struct list_node *prev, struct list_node *n, struct list_node *next)
{
    prev->next = n;
    n->prev = prev;
    n->next = next;
...
#define list_for_each_rev(h, i, member)                                                 \
    for (i = container_of_var(list_debug(h)->n.prev, i, member); &i->member != &(h)->n; \
 ...
 *	parent->num_children = 0;
 */
static inline void list_append_list(struct list_head *to, struct list_head *from)
{
 ...
 *	parent->num_children = 0;
 */
static inline void list_prepend_list(struct list_head *to, struct list_head *from)
{
    struct list_node *from_tail = list_debug(from)->n.prev;
...
#define list_for_each_safe_off(h, i, nxt, off)                               \
    for (i = list_node_to_off_(list_debug(h)->n.next, (off)),                \
        nxt = list_node_to_off_(list_node_from_off_(i, (off))->next, (off)); \
         ...
         i = nxt, nxt = list_node_to_off_(list_node_from_off_(i, (off))->next, (off)))
...
static inline void *list_entry_or_null(const struct list_head *h, const struct list_node *n,
                                       size_t off)
{
    if (n == &h->n)
        return NULL;
...
#define slist_off_(type, member) \
    (offsetof(type, member) + check_type(((type *)0)->member, struct slist_node))

#define slist_pop(h, type, member) ((type *)slist_pop_((h), slist_off_(type, member)))
...
#define slist_for_each(h, pos) for ((pos) = (h)->head.next; (pos); (pos) = (pos)->next)

#define slist_for_each_prev(h, pos, ppos) \
    for ((ppos) = &(h)->head; ((pos) = ((ppos)->next)) != NULL; (ppos) = (ppos)->next)
...
#define hlist_for_each(h, pos) for ((pos) = (h)->head; (pos); (pos) = (pos)->next)
