...
extern void logk_bug(bool fatal, const char *expr, const char *file, int line, const char *func);
...
#define __build_assert_if_constant(cond)                                            \
    _Static_assert(__builtin_choose_expr(__builtin_constant_p(cond), (cond), true), \
...
#define assert(cond)                                                    \
    do {                                                                \
        ...
        if (unlikely(!(cond))) {                                        \
            logk_bug(true, __cstr(cond), __FILE__, __LINE__, __func__); \
...
#define BUG()                                                  \
    do {                                                       \
        logk_bug(true, "false", __FILE__, __LINE__, __func__); \
        __builtin_unreachable();                               \
    } while (0)
 ...
 */
#define WARN() logk_bug(false, "false", __FILE__, __LINE__, __func__);
...
#define WARN_ON(cond)                                                    \
    ({                                                                   \
        ...
        if (unlikely(__result))                                          \
            logk_bug(false, __cstr(cond), __FILE__, __LINE__, __func__); \
        __result;                                                        \
    })
 ...
 */
#if !defined(__CHECKER__) && !defined(__cplusplus)
#define BUILD_ASSERT(cond) _Static_assert(cond, "build-time condition failed")
#else /* __CHECKER__ */
