...
static inline uint64_t eth_addr_to_uint64(struct eth_addr *addr)
{
    ...
    for (i = 0; i < ETH_ADDR_LEN; i++) val |= (addr->addr[i] << (i * 8));
...
static inline void uint64_to_eth_addr(uint64_t val, struct eth_addr *addr)
{
    ...
    for (i = 0; i < ETH_ADDR_LEN; i++) addr->addr[i] = ((val >> (i * 8)) & 0xff);
}
...
#define ETHTYPE_PLANNING 0x8044     /* Planning Research Corp. */
                                    /* 0x8046 - 0x8047	   AT&T */
#define ETHTYPE_DECAM                                                                           \
    0x8048 /* DEC Availability Manager for Distributed Systems DECamds (but someone at DEC says \
              not) */
...
#define ETHTYPE_TCPSM                                                                             \
    0x9002                    /* 3Com (Formerly Bridge Communications), TCP/IP Systems Management \
                               */
