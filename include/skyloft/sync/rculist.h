 ...
 * @n: the node to add
 */
static inline void rcu_hlist_add_head(struct rcu_hlist_head *h, struct rcu_hlist_node *n)
{
...
#define rcu_hlist_for_each_safe(h, pos, tmp, check)           \
    for ((pos) = rcu_dereference_protected((h)->head, check); \
         (pos) && ((tmp) = rcu_dereference_protected((pos)->next, check), 1); (pos) = (tmp))
