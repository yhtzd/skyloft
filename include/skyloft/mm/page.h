...
#define PAGE_END_ADDR  (PAGE_BASE_ADDR + LGPAGE_META_ENTS * PGSIZE_2MB * MAX_NUMA)
...
{
    return ((uintptr_t)addr >= PAGE_BASE_ADDR && (uintptr_t)addr < PAGE_END_ADDR);
}
 ...
 * Returns a numa node
 */
static inline int addr_to_numa_node(void *addr)
{
    return ((uintptr_t)addr - PAGE_BASE_ADDR) / LGPAGE_NODE_ADDR_LEN;
}

#define LGPGN(addr) PGN_2MB((uintptr_t)(addr) - (uintptr_t)PAGE_BASE_ADDR)
#define PGN(addr)   PGN_4KB((uintptr_t)(addr)-PGADDR_2MB(addr))
...
extern int page_init_percpu(void);
