...
struct rte_ether_addr broadcast_mac = {.addr_bytes = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}};
 ...
 */
static inline int port_init(uint8_t port, struct rte_mempool *mbuf_pool, unsigned int n_queues)
{
    ...
    printf("Port %u MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8
           "\n",
           (unsigned)port, my_eth.addr_bytes[0], my_eth.addr_bytes[1], my_eth.addr_bytes[2],
           my_eth.addr_bytes[3], my_eth.addr_bytes[4], my_eth.addr_bytes[5]);
...
static bool check_eth_hdr(struct rte_mbuf *buf)
{
    ...
    if (ptr_mac_hdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) {
        ...
        a_hdr = rte_pktmbuf_mtod_offset(buf, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
...
static void do_client(uint8_t port)
{
    ...
    if (rte_eth_dev_socket_id(port) > 0 && rte_eth_dev_socket_id(port) != (int)rte_socket_id())
        printf(
            "WARNING, port %u (socket %d) is on remote NUMA node to polling thread (socket %d).\n\t"
...
got_mac:
    ...
    found_match:
        ...
        next_send_time +=
            (interval_cycles + (rand() % max_random_cycles) - max_random_cycles * 0.5);
    ...
    if (output_filename != NULL) {
        ...
        for (i = reqs * 0.1; i < reqs * 0.9; i++) {
            float time_us =
                ((float)(rcv_times[i] - snd_times[i]) * 1000 * 1000) / rte_get_timer_hz();
            fprintf(outfile, "%d,%f\n", i, time_us);
        }
        fclose(outfile);
    }
    ...
    for (;;) {
        for (q = 0; q < num_queues; q++) {
            ...
            for (i = 0; i < nb_rx; i++) {
                 ...
                 * ports */
                control_req = rte_pktmbuf_mtod_offset(
                    buf, struct nbench_req *,
                ...
                if (control_req->magic == kMagic) {
                    ...
                    payload_len =
                        sizeof(struct nbench_resp) + sizeof(uint16_t) * control_req->nports;
                    rte_udp_hdr->dgram_len =
                        rte_cpu_to_be_16(sizeof(struct rte_udp_hdr) + payload_len);
                    ptr_ipv4_hdr->total_length = rte_cpu_to_be_16(
...
static int dpdk_init(int argc, char *argv[])
{
    ...
    rx_mbuf_pool = rte_pktmbuf_pool_create("MBUF_RX_POOL", NUM_MBUFS, MBUF_CACHE_SIZE, 0,
    ...
    /* Creates a new mempool in memory to hold the mbufs. */
    tx_mbuf_pool = rte_pktmbuf_pool_create("MBUF_TX_POOL", NUM_MBUFS, MBUF_CACHE_SIZE, 0,
                                           RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
...
static void test_rx(void)
{
    ...
    for (int i = 0; i < nb_rx; i++) {
        ...
        ptr_mac_hdr = rte_pktmbuf_mtod(buf, struct rte_ether_hdr *);
        printf("RX: %d\n", buf->data_len);
        for (int j = 0; j < buf->data_len; j++) printf(" %02x", *((uint8_t *)ptr_mac_hdr + j));
        printf("\n");
        rte_pktmbuf_free(buf);
    }
}
